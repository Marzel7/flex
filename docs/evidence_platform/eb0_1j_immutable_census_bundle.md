...
The bundle contains exactly `run.json`, `aggregate.json`, `corpora.json`, and `hashes.json`. Run metadata binds the census schema, high-water, 5,000-mint ceiling, input fingerprint, result digest, and source-schema fingerprints. Aggregate output records the total eligible count, selected count, older cohort-bound exclusions, coverage, quality, completeness, conflicts, missing valuation, missing event kinds, and uncovered selected mints. Corpus output contains only canonical EB0.1D/EB0.1E projections and their provenance digests—not raw database rows or provider payloads.
