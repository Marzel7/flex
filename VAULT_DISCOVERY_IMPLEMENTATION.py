...
# Constants
SPL_TOKEN_PROGRAM_ID = "TokenkegQfeZyiNwAJsyFbPVwwQQYoQ3ZNrfin2qJAd"
SYSTEM_PROGRAM_ID = "11111111111111111111111111111111"
WRAPPED_SOL_MINT = "So11111111111111111111111111111111111111112"

RAYDIUM_PROGRAM_ID = "675kPX9MHTjS2zt1qrXjVVJJqHHynqvWaWktaDNCqkVV"
ORCA_PROGRAM_ID = "whirLbMiicVdio4KfUqKVrgQTIPbChf3xfghtool2bo"
PUMPSWAP_PROGRAM_ID = "6EF8rrecthR5Dkz92Rayye4g6LJjy5dB3jZpucDdvS3f"
PUMPFUN_PROGRAM_ID = "6EF8rrecthR5Dkz92Rayye4g6LJjy5dB3jZpucDdvS3f"  # May vary
...
async def validate_token_accounts(
    ...
    try:
        ...
        for i, acct in enumerate(accounts):
            ...
            # Check 2: Owner = SPL Token program
            if acct.owner != SPL_TOKEN_PROGRAM_ID:
                logger.debug(f"[VAULT_DISCOVERY] ❌ {address[:16]}... - wrong owner: {acct.owner}")
                continue

            # Check 3: Size = 165 bytes
            if len(acct.data) != 165:
                logger.debug(f"[VAULT_DISCOVERY] ❌ {address[:16]}... - wrong size: {len(acct.data)} bytes (expected 165)")
                continue
...
async def resolve_quote_vault_fallback(
    ...
    """
    Fallback: Discover quote vault via pool registry queries.

    Scans known pool programs for a pool containing the base_vault_address,
    then extracts the quote vault from pool metadata.
    ...
    Returns:
        Quote vault address if found, None otherwise
    """
    logger.debug("[VAULT_DISCOVERY] Attempting fallback quote vault discovery via pool registry")

    for program_id in KNOWN_POOL_PROGRAMS:
        try:
            # Query pools for this program (implementation varies per program)
            pools = await _query_pool_registry(program_id, token_mint, rpc_client)

            for pool in pools:
                if pool.get("base_vault") == base_vault_address:
                    quote_vault = pool.get("quote_vault")
                    if quote_vault:
                        logger.info(f"[VAULT_DISCOVERY] Quote vault found via fallback: {quote_vault[:16]}...")
                        return quote_vault

        except Exception as e:
            logger.debug(f"[VAULT_DISCOVERY] Pool registry query failed for {program_id}: {e}")
            continue
...
async def _decode_pumpswap_pool(data: bytes, rpc_client) -> Optional[str]:
    """Decode PumpSwap pool account and extract quote vault."""
    # TODO: Implement PumpSwap pool decoding
    return None
...
async def register_vault_pair(
    ...
    try:
        ...
        """, (
            ...
            base_vault.decoded.decimals if hasattr(base_vault.decoded, 'decimals') else 6,
            quote_vault.get("decoded", {}).decimals if quote_vault.get("decoded") else 9,
