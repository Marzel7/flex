...
class PoolDiscovery:
    ...
    async def register_pool_to_db(
        ...
        try:
            ...
            def _do_db_write():
                # The listener write lane must be released on every parameter,
                # execute and commit exception path, not only after a clean commit.
                with managed_db_connect(self.db_path, timeout=15) as conn:
                    cursor = conn.cursor()
