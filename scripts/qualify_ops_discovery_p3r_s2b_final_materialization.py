...
def main() -> int:
    ...
    ap.add_argument('--variants',default='qualified,baseline',help='comma-separated: qualified,baseline,qualified_guarded,qualified_guarded_100k')
    args=ap.parse_args()
    ...
    variants={'qualified':(-262144,'OFF',False,1000,1),'baseline':(None,'FULL',False,1000,1),'qualified_guarded':(-262144,'OFF',True,1000,1),'qualified_guarded_100k':(-262144,'OFF',True,100000,1)}
    ...
    for name in selected:
        cache_size,synchronous,guarded,progress_opcode_interval,commit_every_batches=variants[name]
        target=outdir/(name+'.sqlite'); target.unlink(missing_ok=True)
        progress_calls=0; guard_deadline=time.monotonic()+3600.0; last_progress=None; max_progress_gap_seconds=0.0
        def progress():
            nonlocal progress_calls,last_progress,max_progress_gap_seconds
            now=time.monotonic(); progress_calls+=1
            if last_progress is not None: max_progress_gap_seconds=max(max_progress_gap_seconds,now-last_progress)
            last_progress=now
            return int(progress_calls>500000 or now>=guard_deadline)
        ...
        if guarded:
            source.set_progress_handler(progress,progress_opcode_interval); dest.set_progress_handler(progress,progress_opcode_interval)
        ...
        for table,fields in SURFACES:
            ...
            read_seconds=insert_seconds=commit_seconds=0.0; rows=commits=batches_since_commit=0; batch=[]
            ...
            while True:
                ...
                rows+=len(chunk); batches_since_commit+=1
                if commit_every_batches and batches_since_commit>=commit_every_batches:
                    commit_start=time.monotonic(); dest.commit(); commit_seconds+=time.monotonic()-commit_start; commits+=1; batches_since_commit=0
            if batches_since_commit or not commit_every_batches:
                commit_start=time.monotonic(); dest.commit(); commit_seconds+=time.monotonic()-commit_start; commits+=1
        ...
        timing['progress_calls']=progress_calls; timing['max_progress_gap_seconds']=max_progress_gap_seconds; timing['progress_opcode_interval']=progress_opcode_interval if guarded else None; timing['file_size_bytes']=target.stat().st_size; results.append(timing)
    source.close()
