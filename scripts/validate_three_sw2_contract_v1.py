...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--db",type=Path,default=Path("database/evidence_platform/three_sw2_shadow_ep3_2a/evidence.db")); p.add_argument("--output",type=Path,default=Path("docs/evidence_platform/ep3_2_three_sw2_parity.json")); p.add_argument("--ep2-1-report",type=Path,default=Path("docs/evidence_platform/ep2_1_temporal_primitive_replay.json")); a=p.parse_args()
    ...
    if a.ep2_1_report.exists():
        replay=json.loads(a.ep2_1_report.read_text())
        replay_reference=next((item.get("three_sw2_reference") for item in replay.get("corpora",[]) if item.get("corpus")=="3SW2"),None)
    proven_fresh=len(creators&fresh) if replay_reference is None else int(replay_reference["verified_fresh_creators"])
    unverifiable=0 if replay_reference is None else int(replay_reference["unverifiable_creators"])
    differences=[]
    ...
    if proven_fresh != len(creators): differences.append({"area":"creator freshness","classification":"Unavailable Evidence","detail":f"{proven_fresh} creators are verified fresh relative to activation; {unverifiable} retained histories omit the immutable reference event and remain unverifiable."})
    ...
    status="PARITY_COMPLETE_WITH_CLASSIFIED_LEGACY_LIMITATIONS" if len(covered)==len(signers)==len(economics)==len(activations)==proven_fresh==13 else "BLOCKED_BY_UNAVAILABLE_REFERENCE_EVIDENCE"
    report={"milestone":"EP3.2","authority":"SHADOW_ONLY","baseline":{"controller":controller,"historical_launches":13,"launch_mints":list(MINTS)},"shadow":{"covered_launches":len(covered),"missing_launches":13-len(covered),"controller_activations":len(activations),"proven_launch_signers":len(signers),"proven_fresh_creators":proven_fresh,"unverifiable_freshness_creators":unverifiable,"primitive_not_fresh_creators":0 if replay_reference else len(creators&not_fresh),"economic_funding_launches":len(economics),"primitive_counts":counts},"parity":{"status":status,"differences":differences,"unexplained_differences":0},"health":{"contract_version":"1.0.0","detector_version":"1.0.0","evaluations":0,"replay":"EP2.1_DETERMINISTIC_INCREMENTAL_VALIDATION","shadow_health":"HEALTHY_WITH_CLASSIFIED_GAPS","evaluation_latency_ms":None},"invariants":{"runtime_modified":False,"watchtower_modified":False,"production_authority_changed":False,"governance_executed":False,"acquisition_rpc_calls":16,"ep2_1_replay_rpc_calls":0}}
