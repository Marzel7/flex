...
def main() -> int:
    ...
    try:
        ...
        capture.commit(); source.close(); capture.close(); require_budget('final_materialization',180.0)
        ...
        staged.close(); out.close(); os.replace(tmp,snapshot); require_budget('replay_and_boundary',30.0)
        ...
        result={**base,'status':'HOLD','failure_reason':'BOUND_EXCEEDED' if bounded else 'EXECUTION_ERROR','exception_type':type(exc).__name__,'exception':str(exc),'progress_calls':calls,'elapsed_seconds':round(time.monotonic()-start,6)}
