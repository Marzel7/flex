#!/usr/bin/env python3
"""
Analyze complete SOL in/out history for ALL token creators.

For each creator address in the database:
1. Check if they have any transaction signatures
2. Analyze all their transactions for SOL transfers
3. Identify inbound sources (who funded them)
4. Identify outbound destinations (who they sent to - treasuries/extraction)
5. Calculate totals and patterns

This is a comprehensive view of creator funding and extraction across the entire system.
"""
...
class CreatorSOLFlowAnalyzer:
    ...
    def __init__(self):
        self.creators = {}  # {creator_address: {mint, is_known}}
        self.load_creators()

    def load_creators(self):
        """Load all creator addresses from all sources"""
        try:
            conn = sqlite3.connect(DB_PATH, timeout=60)
            cursor = conn.cursor()

            # Get from earliest_tx_creator (most comprehensive)
            cursor.execute(
                "SELECT DISTINCT earliest_tx_creator, mint FROM token_analysis WHERE earliest_tx_creator IS NOT NULL"
            )
            for creator, mint in cursor.fetchall():
                if creator not in self.creators:
                    self.creators[creator] = {"mints": [], "sources": "earliest_tx_creator"}
                self.creators[creator]["mints"].append(mint)

            conn.close()
            print(f"[INIT] Loaded {len(self.creators)} unique creators from earliest_tx_creator\n")
    ...
    async def analyze_creator_sol_flow(
        ...
        for sig in signatures:
            try:
                ...
                print(f"[TX_ERROR] {sig[:16]}...: {str(e)[:50]}")
                continue
    ...
    async def run(self):
        ...
        try:
            ...
            for idx, creator in enumerate(sorted(self.creators.keys()), 1):
                result = await self.process_creator(creator, idx, len(self.creators))
            ...
            if creators_with_inbound > 0:
                ...
                for r in top_inbound:
                    ...
                    print(
                        f"  {creator_short}: {r['inbound_total']:.6f} SOL from {len(r['inbound'])} source(s)"
                    )
            ...
            if creators_with_outbound > 0:
                ...
                for r in top_outbound:
                    creator_short = f"{r['creator'][:8]}...{r['creator'][-4:]}"
                    print(
                        f"  {creator_short}: {r['outbound_total']:.6f} SOL to {len(r['outbound'])} destination(s)"
                    )
    ...
    async def _store_results(self, results: List[Dict]):
        ...
        try:
            ...
            cursor.execute(
                """
                CREATE TABLE IF NOT EXISTS creator_sol_flow_analysis (
            ...
            for r in results:
                ...
                cursor.execute(
                    """INSERT OR REPLACE INTO creator_sol_flow_analysis
                       (creator_address, transaction_count, inbound_sol, inbound_sources,
